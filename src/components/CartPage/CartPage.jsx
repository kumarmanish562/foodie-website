...
const CartPage = () => {
  ...
  return (
    <div className='min-h-screen overflow-x-hidden py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d]'>
                  ...
                  <div className='flex items-center gap-3'>
                    <button onClick={()=> updateQuantity(item.id, Math.max(1, item.quantity -1))}
                      className=' w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-all duration-300 active:scale-95'>
                        <FaMinus className='w-8 h-4  text-amber-100 '/>
                      </button>
                      ...
                      className=' w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-all duration-300 active:scale-95'>
                        <FaPlus className='w-8 h-4  text-amber-100 '/>
                      </button>
                       </div>
      ...
      {selectImage && (
        <div className=' fixed inset-0 z-50 flex items-center justify-center bg-amber-900/40 bg-opacity-75 backdrop-blur-sm p-4 overflow-auto' onClick={() =>
